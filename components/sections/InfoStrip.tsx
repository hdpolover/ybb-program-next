  ...
  const items = [
    {
      ...
      icon: <MapPin className={jysSectionTheme.infoStrip.icon} />,
    },
    {
      subtitle: 'Contact',
      title: 'Customer Service',
      description: '+6285173386622',
      icon: <Phone className={jysSectionTheme.infoStrip.icon} />,
    },
    {
      subtitle: 'Email',
      title: 'Official Email',
      description: 'japanyouthsummit@gmail.com',
      icon: <Mail className={jysSectionTheme.infoStrip.icon} />,
    },
  ];

  return (
    <section className={`${jysSectionTheme.infoStrip.background} relative w-full overflow-hidden`}>
      ...
      <div className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        <ul className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => (
            <li
              key={item.subtitle}
              className={`flex items-start gap-6 lg:px-10 ${index !== items.length - 1 ? `lg:border-r ${jysSectionTheme.infoStrip.divider}` : ''}`}
            >
              <div className={jysSectionTheme.infoStrip.iconCircle}>{item.icon}</div>
              <div>
                <p className={jysSectionTheme.infoStrip.subtitle}>{item.subtitle}</p>
