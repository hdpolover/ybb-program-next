...
import { ChevronDown, LogOut, Repeat2, UserCircle2 } from 'lucide-react';
...
export default function UserMenuPopover({
  ...
  const handleSwitchMode = () => {
    const next = mode === 'ambassador' ? 'participant' : 'ambassador';
    setMode(next);
    // navigate to the right home for the new mode
    router.push(next === 'ambassador' ? '/dashboard/ambassador' : '/dashboard');
    setOpen(false);
  };
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-64 rounded-2xl border border-slate-200 bg-white py-2 shadow-xl">
          ...
          <div className="py-1">
            {/* Mode switcher — only shown to ambassadors */}
            {isAmbassador && (
              <button
                type="button"
              ...
              >
                ...
                <div className="text-left">
                  <p className="font-medium">
                    {mode === 'ambassador' ? 'Switch to Participant View' : 'Switch to Ambassador View'}
                  </p>
                  <p className="text-xs text-slate-400">
                    {mode === 'ambassador'
                      ? 'View your applications and payments'
