...
export async function GET(request: Request) {
  try {
    ...
    const apiUrl = new URL('/v1/portal/documents', (process.env.API_INTERNAL_URL || process.env.NEXT_PUBLIC_API_URL || 'https://staging-api.ybbhub.com').replace(/\/v1\/?$/, ''));
    if (programId) {
      ...
      return NextResponse.json(
        {
          ...
          message: (json as any)?.message ?? 'Failed to fetch documents',
